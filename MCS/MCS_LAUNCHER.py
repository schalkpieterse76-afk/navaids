...
def save_config(mode: str, ip: str, port: int) -> dict:
    cfg = {
        ...
        "server_ip":    ip,
        "server_port":  port,
        "installed_at": _dt.now(_dt.timezone.utc).isoformat(),
    }
    ...
    def __init__(self, root, on_show, on_quit, on_reconfigure=None):
        ...
        try:
            ...
            menu = pystray.Menu(
                ...
                pystray.MenuItem('Reconfigure…',  lambda: root.after(0, on_reconfigure or (lambda: None))),
...
def main():
    ...
    # ── Step 5: start backend (server mode only) ──────────────────────────────
    ...
    if args.browser_only:
        url = f"http://{srv_ip}:{srv_port}" if mode == "server" else html_path.as_uri()
        webbrowser.open(url)
        return
